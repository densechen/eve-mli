...
class FormatUnsupportedError(NotImplementedError):
    def __init__(self, unsupported_formats: Sequence[str],
...
class HumanOutputFormat(KVWriter, SeqWriter):
    def __init__(self, filename_or_file: Union[str, TextIO]):
        ...
        if isinstance(filename_or_file, str):
            ...
            assert hasattr(
                filename_or_file,
                "write"), f"Expected file or str, got {filename_or_file}"
    ...
    def _truncate(cls, string: str, max_length: int = 23) -> str:
        return string[:max_length -
                      3] + "..." if len(string) > max_length else string
...
def filter_excluded_keys(key_values: Dict[str, Any],
                         key_excluded: Dict[str, Union[str, Tuple[str, ...]]],
                         _format: str) -> Dict[str, Any]:
    """
    ...
    def is_excluded(key: str) -> bool:
        return key in key_excluded and key_excluded[
            key] is not None and _format in key_excluded[key]

    return {
        key: value
        for key, value in key_values.items() if not is_excluded(key)
    }
...
class JSONOutputFormat(KVWriter):
    ...
    def write(self,
        ...
        key_values = {
            ...
            for key, value in filter_excluded_keys(key_values, key_excluded,
                                                   "json").items()
        }
...
def make_output_format(_format: str,
                       log_dir: str,
...
class Logger(object):
    ...
    def record_mean(
        ...
        self.name_to_value[key] = old_val * count / (count +
                                                     1) + value / (count + 1)
...
def configure(folder: Optional[str] = None,
    ...
    if format_strings is None:
        format_strings = os.getenv("SB3_LOG_FORMAT",
